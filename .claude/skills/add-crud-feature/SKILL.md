...
- `src/app/(dashboard)/dashboard/settings/organization/page.tsx` — server component reading data with Drizzle + `requireOrganization()`.
- `src/app/(dashboard)/dashboard/settings/billing/billing-actions.tsx` + `billing/page.tsx` — pattern for a server page + a `"use client"` actions component.
...
3. **Server actions**: create `src/app/(dashboard)/dashboard/<resource-plural>/actions.ts` with a `"use server"` directive. Implement `create<Resource>`, `update<Resource>`, `delete<Resource>`, `list<Resource>s`. Every query/mutation must:
...
4. **UI**:
   - `src/app/(dashboard)/dashboard/<resource-plural>/page.tsx` — server component, fetches rows via the list action, renders `<<Resource>Table>`.
